...
uint32_t nco_178k(uint16_t write_bc_ticks, uint16_t *ff_samples, size_t ff_sample_count, uint32_t *bc_buf, uint32_t bc_bufmask)
{
    ...
    // f_n = 144,000 Hz   (allows for 500Hz offset on 250kHz MFM clock, 1kHz on 500kHz, and 2kHz on 1MHz)
    // k_l = 0.025_132_741_23 ~= 1/32
    // k_i = 0.000_157_913_670_4  ~= 1/8192

    // k_l = 1/32    ~= f_n 179kHz
    // k_i = 1/8192  ~= f_n 126kHz
    ...
    for (int ii = 0; ii < ff_sample_count; ++ii)
    {
        ...
        //printf("P: %10d I: %10d ", phase_error/32, phase_integral/8192);

        phase_step = (uint32_t)((int32_t)(1 << 16) + phase_integral/8192 + phase_error/32);
