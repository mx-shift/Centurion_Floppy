...
uint32_t nco_2160k_0p25(uint16_t write_bc_ticks, uint16_t *ff_samples, size_t ff_sample_count, uint32_t *bc_buf, uint32_t bc_bufmask, struct kv_pair *params)
{
    // dma_wr struct
    uint32_t phase_step;
    ...
    // zeta = 0.25
    // f_n = 2,160,000 Hz
    // k_l = 0.09424777961 ~= 3/32
    // k_i = 0.03553057584 ~= 1/32
    ...
    for (int ii = 0; ii < ff_sample_count; ++ii)
    {
        ...
        phase_step = (uint32_t)((int32_t)(1 << 16) + phase_error*3/32 + phase_integral/32);
...
struct algorithm algorithm_nco_2160k_0p25 = {
    .name = "nco_2160k_0p25",
    .func = nco_2160k_0p25,
    .params = NULL,
};